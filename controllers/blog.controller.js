...
exports.getOneBlog = async (req, res, next) => {
  ...
  try {
    const blog = await Blog.findOne({ deleted_at: null, id: blog_id });
    if (!blog) {
...
exports.updateBlog = async (req, res, next) => {
  ...
  try {
    ...
    if (req.file) {
      fs.unlink(path.join(require.main.path, cover_picture_url), (err) => {
    ...
    await Blog.updateOne(
      { id: blog.id },
      { $set: { title, description, cover_picture_url } }
    );
